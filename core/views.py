...
def order_create(request):
    ...
    if form.is_valid():
        ...
        customer, _ = Customer.objects.get_or_create(
            ...
            defaults={'name':data['buyer_name'],'city':data.get('buyer_city',''),'area':data.get('buyer_area',''),'address':data.get('buyer_address','')}
        )
        changed=False
        for field, key in [('name','buyer_name'),('city','buyer_city'),('area','buyer_area'),('address','buyer_address')]:
            val=data.get(key) or ''
            if val and getattr(customer, field) != val:
                setattr(customer, field, val); changed=True
        if changed: customer.save()
        product=data.get('product')
        if not product:
            product, _ = Product.objects.get_or_create(
                sku=data['sku'],
                defaults={'name':data['product_name'],'selling_price':data['unit_price'],'cost_price':0,'stock_quantity':0,'low_stock_alert':0}
            )
        obj=Order.objects.create(
            customer=customer, product=product, quantity=data['quantity'], unit_price=data['unit_price'],
            discount=data.get('discount') or 0, delivery_company=data.get('delivery_company'), delivery_fee=data.get('delivery_fee') or 0,
            status=data['status'], payment_method=data['payment_method'], notes=data.get('notes') or '', assigned_to=request.user
        )
        amount=data.get('amount_paid') or Decimal('0')
        if amount > 0:
            Payment.objects.create(order=obj, amount=amount, method=data['payment_method'], notes='دفعة مسجلة مع إنشاء الطلب')
        log(request,'إضافة طلب جديد','Order',obj.id)
        messages.success(request,'تم إنشاء الطلب بدون أي بيانات تجريبية')
...
def export_orders_xlsx(request):
    start=request.GET.get('start',''); end=request.GET.get('end',''); status=request.GET.get('status','')
    qs=Order.objects.select_related('customer','product','delivery_company').prefetch_related('payments')
    if start: qs=qs.filter(order_date__date__gte=start)
    if end: qs=qs.filter(order_date__date__lte=end)
    if status: qs=qs.filter(status=status)
    rows=[]
    for o in qs:
        methods=' / '.join(sorted(set([p.get_method_display() for p in o.payments.all()]))) or o.get_payment_method_display()
        rows.append([o.id,o.order_date.strftime('%Y-%m-%d'),o.customer.name,o.customer.phone,o.customer.city,o.product.name,o.quantity,o.get_status_display(),o.get_payment_method_display(),o.get_payment_status_display(),methods,o.delivery_company.name if o.delivery_company else '',float(o.total_amount),float(o.paid_amount),float(o.remaining_amount),o.notes])
    return _xlsx_response('orders_report.xlsx',['ID','Date','Customer','Phone','City','Product','Qty','Status','Agreed Payment Method','Payment Status','Collected Method(s)','Delivery Company','Total','Paid','Remaining','Notes'],rows)

@login_required
def export_inventory_xlsx(request):
    rows=[[p.sku,p.name,p.category,p.stock_quantity,p.low_stock_alert,p.stock_status,float(p.selling_price),float(p.cost_price)] for p in Product.objects.all()]
    return _xlsx_response('inventory_report.xlsx',['SKU','Product','Category','Current Stock','Low Stock Alert','Status','Selling Price','Cost Price'],rows)

@login_required
def export_receivables_xlsx(request):
    qs=Order.objects.select_related('customer','product','delivery_company').exclude(payment_status='paid').exclude(status__in=['cancelled','rejected'])
    rows=[[o.id,o.order_date.strftime('%Y-%m-%d'),o.customer.name,o.customer.phone,o.product.name,o.get_payment_method_display(),o.delivery_company.name if o.delivery_company else '',float(o.total_amount),float(o.paid_amount),float(o.remaining_amount),o.get_status_display()] for o in qs if o.remaining_amount>0]
    return _xlsx_response('receivables_report.xlsx',['Order ID','Date','Customer','Phone','Product','Receivable Type','Delivery Company','Total','Paid','Remaining','Status'],rows)
