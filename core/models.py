...
class Order(models.Model):
    ...
    def release_stock(self):
        exists = self.inventory_movements.filter(movement_type='out').exists()
        if not exists:
            InventoryMovement.objects.create(
                ...
                notes=f'خروج تلقائي من أوردر #{self.id}'
            )
    ...
    def return_stock(self):
        ...
        if has_out and not has_return:
            InventoryMovement.objects.create(
                ...
                quantity=self.quantity,
                notes=f'رجوع تلقائي من أوردر #{self.id}'
            )
    ...
    def sync_auto_payment(self):
        ...
        if self.payment_method in paid_methods and self.total_amount > 0:
            Payment.objects.create(
                ...
                notes='دفعة تلقائية من صفحة الأوردرات اليومية'
            )
...
class InventoryMovement(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        Product.objects.filter(pk=self.product_id).update(
            stock_quantity=models.F('stock_quantity') + diff
        )

    def delete(self, *args, **kwargs):
        Product.objects.filter(pk=self.product_id).update(
            stock_quantity=models.F('stock_quantity') - self.signed_quantity
        )
        ...
        if self.cashbox_id:
            CashTransaction.objects.update_or_create(
                ...
                defaults={
                    ...
                    'transaction_type': 'in',
                    'amount': self.amount,
                    'description': f'تحصيل أوردر #{self.order_id} - {self.get_method_display()}',
