...
class Order(models.Model):
    STATUS_CHOICES = [
        ...
        ('with_delivery', 'تم تسليمه لشركة التوصيل'),
    ...
    payment_status = models.CharField('حالة الدفع', max_length=30, choices=PAYMENT_STATUS_CHOICES, default='unpaid', db_index=True)
    ...
    def release_stock(self):
        if not self.inventory_movements.filter(movement_type='out').exists():
    ...
    def return_stock(self):
        has_out = self.inventory_movements.filter(movement_type='out').exists()
        has_return = self.inventory_movements.filter(movement_type='return').exists()
        if has_out and not has_return:
    ...
    def refresh_payment_status(self):
        ...
        if self.payment_status != status:
            Order.objects.filter(pk=self.pk).update(payment_status=status, updated_at=timezone.now())
            self.payment_status = status

    def sync_auto_payment(self):
        self.payments.filter(notes='دفعة تلقائية من صفحة الأوردرات اليومية').delete()
        paid_methods = ['cash', 'cliq', 'bank', 'card', 'wallet', 'other']
        if self.payment_method in paid_methods and self.total_amount > 0:
            Payment.objects.create(order=self, amount=self.total_amount, method=self.payment_method, notes='دفعة تلقائية من صفحة الأوردرات اليومية')
        self.refresh_payment_status()

    def apply_order_effects(self):
        if self.status in ['with_delivery', 'delivered']:
            self.release_stock()
        if self.status in ['returned', 'cancelled', 'rejected']:
            self.return_stock()
...
class Payment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.cashbox_id and self.method not in ['unpaid', 'delivery']:
            ...
            self.cashbox, _ = CashBox.objects.get_or_create(name=f'صندوق {label}')
        super().save(*args, **kwargs)
        if self.cashbox_id:
